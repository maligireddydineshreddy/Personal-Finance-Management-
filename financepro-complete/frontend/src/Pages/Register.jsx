...
import { FaEnvelope, FaLock, FaChartLine, FaUser, FaRupeeSign } from "react-icons/fa";
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate salary
    const cleanSalary = removeCommas(salary);
    if (isNaN(cleanSalary) || parseFloat(cleanSalary) <= 0) {
    ...
    try {
      const response = await axios.post(
        `${API_CONFIG.BACKEND_URL}/users/signup`,
        {
          name,
          email,
          password,
      ...
      setIsLoading(false);
      const errorMsg = err.response?.data?.message || "Error registering user. Please try again.";
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <FaUser className={`text-white/60 transition-colors ${nameFocused ? 'text-white' : ''}`} />
              </div>
              <input
            ...
            {/* Salary Input with Floating Label */}
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <FaRupeeSign className={`text-white/60 transition-colors ${salaryFocused ? 'text-white' : ''}`} />
              </div>
              <input
                className="w-full pl-12 pr-4 py-4 bg-white/10 backdrop-blur-sm border-2 border-white/20 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-white/50 focus:bg-white/15 transition-all duration-300"
                id="salary"
                type="text"
                placeholder=" "
                value={formatNumberWithCommas(salary)}
                onChange={(e) => {
                ...
                onBlur={() => setSalaryFocused(!salary)}
                required
              />
              <label
                className={`absolute left-12 transition-all duration-300 pointer-events-none ${
                  salaryFocused || salary
                    ? "top-2 text-xs text-white/90 font-semibold"
                    : "top-4 text-white/60"
                }`}
                htmlFor="salary"
